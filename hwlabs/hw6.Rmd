...
1. Repeat [Homework 5](hw5.html) Question 1a and 1b, but this time using only `purrr` functions instead of loops or vectorized operations (`map_dbl`, etc. You may also use `sum`). Compare the run times with your previous results using loops and vectorized operations and state what you find (e.g., which approach is the fastest, which is the slowest).
...
1. Perform the following using `purrr` functions as much as possible.
    ...
    a. Find the variable type (using `class`) of each column in `classdata::cities`. The result should be a vector.
...
1. Work on a list containing information of Game of Thrones characters. Install `repurrrsive` using `install.packages("repurrrsive")`, and load it using `library(repurrrsive)`. The list `got_chars` is what you will use. Check out what does it contain before you start.
    ...
    a. Create a new file for each character. The file name should be `characterName.txt`, replacing the corresponding character name, and the content should be just the name of the actor. Use `writeLines(charVec, fileName)` to write the contents of a character vector `charVec` into a file `fileName`.
    a. Create a new file containing all the information for each character. 
    The file name should be `characterNameAll.txt`, replacing the corresponding character name. [Hint: You can use RStudio to open the `.txt` files and check your results. Click File -- Open File] 
    
        Here is an example content for the first character:
...
1. Reason with each of the following code (run them in your head). Make a prediction of the output before running the code. Use the principles (could be more than 1) covered in [the lecture](../slides/02_04_advanced_functions.html) to explain why the output makes sense.
...
for $n \ge 0$. The user will input a vector `a` specifying the coefficients (`a[1]`=$a_0$, `a[2]`=$a_1$, etc), a lower bound `lower` ($l$), and an upper bound `upper` ($u$). 
    ...
    You function should perform the integral *numerically* using the following steps (instead of applying integral rules learned in Calculus):
    ...
    b. apply the `integrate` function on the manufactured polynomial function
    
    After you are done writing the function, test that it ...  TODO
    
    [Hint: This is an extensive question, so break in down into multiple steps. Use some unit tests to make sure a step is completely correct *before* moving onto the next. ]
...
$$l(\alpha, \beta) = \sum_{i=1} \log f(X_i;\,\alpha,\beta).$$
    ...
    Implement a function factory to manufacture the log-likelihood function. The function factor should take a vector `x` input for the dataset and output the log-likelihood function corresponding to `x`. The manufactured function should take a single input `par`, which is a vector of length 2 where the first entry stands for `gamma` and the second entry stands for `beta`.
    
    a. Implement the function factory
    b. Manufacture a log-likelihood function using input `c(0.289, 0.468, 0.035, 2.114, 0.281)`
    c. Plot the log-likihood function you manufactured, fixing $\alpha=1$ and let $\beta$ range from 0.1 to 2

    [Hint: You can use `lgamma` to find the log of the gamma function. The reason to always operate on the log scale when solving optimization involving likelihood is that when dataset becomes large, the likelihood function can get very large or small, so calculation can lose considerable precision. ]
...
1. (Optional) Use the `optim` function to find the maximum likehood estimation for the following dataset
    ...
    ```
assuming they come from a gamma distribution.
