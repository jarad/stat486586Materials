...
- Widely appliable: 
  ...
  - Permutation tests work for any finite sample size, unlike the bootstrap and parametric tests which rely on asymptotics
...
- This means any choice of 7 values from the combined data is equal likely  to form the treatment group; the rest form the control group

- In summary, under $H_0$, any permutation of the pooled values is equally likely to be observed
...
To make idea precise, here is a toy two-sample test example. Sample 1 is $(X_1,X_2) = (1,3)$ and Sample 2 is $Y_1 = 9$. Consider test statistic 
...
1. Choose a two-sample test statistic $T(x_1,\dots,x_{n_1},y_1,\dots,y_{n_2})$. E.g., the difference in the sample mean, or the ratio of sample means, etc
...
- We are actually testing $H_0:$ $X$ and $Y$ are independent, since we are simulating that scenario (picking which $X$ is independent of $Y$)
...
Test hypothesis about the center $\mu$:
