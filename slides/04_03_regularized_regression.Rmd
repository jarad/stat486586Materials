...
plot(log(ridgeTrain$lambda), testMSE, type="l")
  ...
  # Default: 10-fold; automatically implemented (no need to write from scratch if you use this function)
...
yPredTrain <- predict(ridgeCV, newx = X[trainInd, , drop=FALSE], 
                      s=ridgeCV$lambda,
                      type="response")
trainMSERidge <- colMeans((yPredTrain - y[trainInd])^2)
yPredTest <- predict(ridgeCV, newx = X[testInd, , drop=FALSE], 
                     s=ridgeCV$lambda,
                     type="response")
testMSERidge <- colMeans((yPredTest - y[testInd])^2)
lines(log(ridgeCV$lambda), trainMSERidge, lty=1)
lines(log(ridgeCV$lambda), testMSERidge, lty=2) 
legend("bottomright", legend=c("CV MSE", "training MSE", "test MSE"), lty=c(1, 1, 2), col=c("red", "black", "black"))
# red: CV estimate of test error; black: test set estimate

# lambda.min better than lambda.1se
...
# In this example, the lambda.min choice is better than lambda.1se, but given a different dataset the relationship may be reversed
