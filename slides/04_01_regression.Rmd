...
In a supervised learning task, you are given data $(X_1,Y_1),\dots,(X_n,Y_n)$, where the $Y_i$ are the response variable, either quantitative for a *regression* problem, or qualitative for a *classification* problem.
...
- Machine learning is a closely related, partially overlapping set of tools for solving the same problems as does statistical learning.
...
In a regression setting, the responses $Y_i$ are continuous, which are related to $p$ predictors $X_{i1},\dots, X_{ip}$.
...
- In both cases, the relationship between the response and each predictor is *linear*. I.e., a unit increase in $X_{i1}$ always results in $\beta_1$ units of increase in $Y_i$, fixing other predictors at *any* level
...
- The solution is the *least squares estimation* of $\boldsymbol{\beta}$ (refer to [a previous lecture](01_11_matrix.html#13)), defined as 
...
- Polynomial regression extends the linear model by adding extra predictors, obtained by raising each of the predictor to a power.

- We model the nonlinear relationship as

$$Y_i = \beta_0 + \beta_1 X_{i}^1 + \dots + \beta_p X_{i}^q + \epsilon_i.$$

- For example, a cubic regression (degree 4) uses three variables, $X_i$, $X_i^2$, and $X_i^3$ as predictors
...
- Unfortunately, the piecewise cubic fit looks bad (discontinuous)
...
TODO: df in bs()
including multiple predictors, some w/ nonlinear effects
Coding covariates (model.matrix)
