...
For someone who had experience with C/C++/Python/etc:

- R is an intuitive high-level programming language

- R has an excellent handling of data, analysis, and graphics. At the same time, it is also general purposed
...
- Each to access to documentation: E.g., type `?cos` in the R console
...
- `:`, for creating a simple sequence of numbers
...
- `vector(mode = "numeric", length=10)`, for initializing a numeric vector of length 10. .style[Initialization can most of the time be avoided in R]
...
- `<-` is the assignment operator. `=` also works as an assignment operator, .style[but please avoid it as a matter of style.] E.g., 
  ```{r}
  x <- c(2, 5)
  ```
...
???
When R is smart enough to tell how the variable should be typed. You don't need to declare types
...
- For reproducibility, you want any meaningful work to be reproducible. Namely, the results should be reproduced by running the **code** you saved, but not by saving the results directly
...
- Note the operator precedence. See `?Syntax`
