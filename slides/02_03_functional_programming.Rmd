...
- .style[You may think you can do everything using loops, but this will prevent you to build big things. Using functional programming, you will gain much more mileage as we learn more ]
...
<img src="images/green.png" width=20> Find the sum of the values in each entry of `l` using `map`
...
```{r}
map(x, function(xx) {
  map_dbl(c(0, 0.3), mean, x=xx)
  # same as: map_dbl(c(0, 0.3), function(trim) mean(xx, trim=trim))
})
```
...
pmap(record, 
     function(student, id, score) {
       stringr::str_c(student, id, ": ", score, sep=" ")
     })
```
...
- `map2_dbl`, `map2_chr`, `map2_lgl`, `walk2` etc are also available

- `pmap_dbl`, `pmap_chr`, `pmap_lgl`, `pwalk` etc are also available
