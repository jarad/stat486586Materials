...
ggplot(fbi2018, aes(x = Type, weight=Count)) + geom_bar() + coord_flip()
```
...
A *side-by-side histogram* works well comparing a small number of distributions. 
...
## Stacked barcharts
A stacked barchart visualizes one discrete (e.g., State) vs one continuous (Count)
...
Use the `fbi` dataset and `ggplot2` package to answer the following questions. 

<img src="images/green.png" width=20> Compare the distribution of the number of robberies in all states: Using ggplot2, display a side-by-side boxplot of the number of robberies by state. What does each data point stand for? 
...
<img src="images/green.png" width=20> Visualize the number of violent crimes in 2018. The x-axis should show the state and the height of the bars should show the number of reports (use `aes(..., weight=)`). 
