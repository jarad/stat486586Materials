...
- Since we have qualitative response, the most common objective in classification is to minimize the *misclassification error rate*
$$E[\mathrm{1}\{Y \ne \hat{Y}\}]$$
...
a fair quantification of the model performance 

---

## Confusion matrix

Given $n$ observations, the confusion matrix is a tabulation of the predicted labels and the true labels
...
- A method that predicts too many positive will have a high sensitivity but low specificity
---
...
- If we have two classes $j \in \{0, 1\}$, the Bayes classifier would predict class 1 if $$P(Y = 1\mid X=X_0) > 1/2$$ and class 0 otherwise

- The error rate made by the Bayes classifier is the *Bayes error rate*. This is analogous to the irreducible error in the regression setting

- The Bayes classifier is unattainable gold standard
...
- Nonetheless, the Bayes classifier offers an idea to frame the classification problem: We target to estimate the conditional probability $P(Y=j \mid X=x)$
...
- Then why bother classification models? 
    ...
    - The major reason lies in the *objectives*. Regression aims to minimize mean squared error (degree of difference), and classification aims to minimize misclassification (same or different, but not not the precise degree of difference). A regression model is optimal for the former but not the latter
    
    - If the response has multiple classes that are not ordered (e.g., diagnosis or ethnicity), regression cannot be used
...
- The $K$-nearest neighbors (KNN) classifier estimates the conditional probability for class $J$ using 
...
- "If the predictors are alike, the response should also be similar"

---

- To make a prediction for $x$ at the cross:</br>
...
- We will be focusing on the Euclidean distance, but there are many other alternatives, e.g. the Manhattan distance (the $l_1$ distance) and the weighted Euclidean distance
...
YHatKNN <- knn(X[trainInd, ], X[testInd, ], y[trainInd])
table(y[testInd], YHatKNN)
# TODO: calc sens, spec
```
...
$$E(Y\mid X=x) = P(Y = 1 | X=x) = \beta_0 + \beta_1 x.$$
Instead, for classification, the logistic regression model is
...
.center[![:scale 50%](images/linearLogistic.jpg)]
...
Interpretation: 
- The log-odds that $Y=1$ occurs is increased by 1 for every unit of increase in $X$

- The odds that $Y=1$ occurs is increased by by $\exp(1) - 1=1.718$ for every unit of increase in $X$

- *If $p$ is very small*, then approximately the probability that $Y=1$ occurs is increased by $1.718$ for every unit of increase in $X$
...
- The coefficient estimates are obtained by the maximum likelihood estimatesion (MLE). 
...
# Should we include income or not? Sometimes, even if a predictor is insignificant in the model (for inference), it could still be helpful for making prediction
...
# CV error with income is only slightly smaller. This may be due to randomness. So it is best not to include it for prediction.
```
