...
Each layer has several parts which can be customized. The two most important ones are:
...
`ggplot` sets the *default* dataset and aesthetics mapping for all downstream layers, and <br>
`geom_XXX` creates a new layer. If data or aesthetics are specified in the layer, they will overwrite the default when necessary
...
## 
- WARNING: Putting `color='blue'` in `aes()` usually does not give you what you want.
