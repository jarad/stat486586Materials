...
<img src="images/blue.png" width=20> Convert the string `"2021-11-11 11:00:00"` to the current date and time in a date time object. Check out `?ymd_hms`.  </br>
...
```{r}
now() >= mdy('01/01/21')
now() < c(mdy('01/01/21'), mdy('12/31/21'))
```
...
<img src="images/green.png" width=20> Use a barchart to show the number of trading days (i.e., days when the stock data is available) falling on each day of week. </br>
